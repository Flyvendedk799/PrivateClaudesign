...
2. **Mechanic spec block** — emit ONE assistant_text block (≤ 120 words, no inline tool calls in the same turn) with EXACTLY this template, on its own line each:
   \`\`\`
...
3. **\`set_todos\`** — Publish the section/scene/system list FIRST. One todo per scene-or-system (e.g. for Pong: "Field + paddles", "Ball physics + collisions", "Score HUD", "Win state + restart"). 4 todos minimum for a complete game. Items ≤ 8 words. **Call set_todos at most twice per turn** — once at the start to publish the plan and once near the end to mark completion. Do NOT update after every single item; mid-run updates are noise.
4. **\`text_editor.create\`** with the engine's \`canonicalEntry\` (\`index.html\` for three/phaser, \`main.py\` for pygame, \`project.godot\` for godot). Use the engine's starter template — do **not** reinvent the import-map, base href, or \`__game\` global shim; those are load-bearing.
5. **\`text_editor.create\` / \`str_replace\`** for the rest of the project — \`src/main.js\` + scenes/ + entities/ + assets/ for JS engines; \`entities.py\` + assets/ for Pygame; \`*.tscn\` + \`*.gd\` + assets/ for Godot.
6. **\`generate_image_asset\`** for sprites and tiles when needed. \`purpose: 'sprite'\` for power-of-two transparent tiles; \`'tile'\` for seamless edges; \`'background'\` for full-bleed.
7. **\`generate_audio_asset\`** for SFX / short music loops / voice cues. \`purpose: 'sfx'\` for clicks/jumps/hits/coins/footsteps/laser/explosion; \`'music'\` for menu jingle or ambient loop; \`'voice'\` for notification chime placeholder. Synchronous + free (CC0 sample bank, no API call). Call once per cue you'll wire into the game; the tool returns \`assets/audio/<name>.wav\` paths your engine's audio loader can reference. **MANDATORY whenever the brief mentions hits / impacts / pickups / shoots / explosions / coins / death / combat** — silent feedback on collision reads as broken (Mechanic-first: "every action gives a visible AND audible response"). The \`assert_game_invariants\` pass before \`done\` will flag the \`feedback\` invariant as missing if you ship audio-bearing gameplay without any audio asset wired in.
8. **\`verify_artifact\`** between scene completions to catch breakage early; it's cheap.
9. **\`validate_game_scene\`** before \`done\` — engine-specific lint (collision detection wired, scene lifecycle present, no orphan asset keys, no \`eval\`).
10. **\`assert_game_invariants\`** before \`done\` — cross-engine sanity check that the four design-level invariants are present: a restart binding, a fail state, a score / state mutation, and an audible/visible feedback cue inside collision handlers. Genre-aware: when the spec block named a brawler, also checks combo + hitstop + per-attack-limb + aim/hitbox parity. Warnings are non-blocking but should be addressed.
11. **\`playtest_game\`** — host-driven synthetic input/mouse playtest that asserts input → state mapping (e.g. KeyD moves player +x; mousedown attacks the entity under the reticle). Call ONCE before \`done\` after \`assert_game_invariants\` passes. Catches the rotation/aim sign-error class that pure rendering checks miss. No-op when the run has no \`window.__game.debug\` getter wired.
12. **\`done\`** — closing call only. Summary explains the mechanic in one sentence + lists controls.
