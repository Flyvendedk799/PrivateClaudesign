...
6. **\`generate_audio_asset\`** for SFX / short music loops / voice cues. \`purpose: 'sfx'\` for clicks/jumps/hits/coins/footsteps/laser/explosion; \`'music'\` for menu jingle or ambient loop; \`'voice'\` for notification chime placeholder. Synchronous + free (CC0 sample bank, no API call). Call once per cue you'll wire into the game; the tool returns \`assets/audio/<name>.wav\` paths your engine's audio loader can reference. **MANDATORY whenever the brief mentions hits / impacts / pickups / shoots / explosions / coins / death / combat** — silent feedback on collision reads as broken (Mechanic-first: "every action gives a visible AND audible response"). The \`assert_game_invariants\` pass before \`done\` will flag the \`feedback\` invariant as missing if you ship audio-bearing gameplay without any audio asset wired in.
