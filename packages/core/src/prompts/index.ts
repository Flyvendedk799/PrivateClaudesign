...
- **Named real-world objects need real assets.** When the spec or user prompt mentions a recognizable real-world item by name — weapons (M4, Desert Eagle, AK-47, katana, knife), vehicles (Ferrari, F-22, Tesla), branded items, named characters — DO NOT build it from procedural primitives (cylinders + boxes + spheres). The FPS Wave Defense user complained "the knife doesnt look like a knife. The m4 doesnt look like a m4. And the desert deagle doesnt look like a desert eagle" because procedural geometry from training data can't approximate recognizable real-world shapes. Instead: call \`generate_image_asset\` with \`purpose: 'sprite'\` (for 2D / billboard textures) or import a GLB the user supplies. Procedural primitives are fine for ABSTRACT shapes (generic platform, blob enemy, ground plane, geometric obstacle) and for retro/pixel-art aesthetics.`;
